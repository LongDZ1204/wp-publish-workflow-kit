...
def validate(bundle: Path, profile: dict, phase: str, allow_pilot: bool = False) -> dict:
    ...
    parser.feed(html)
    # H1 ownership is a per-site choice declared in publish-context.json:
    # 1 when the body owns the page H1, 0 when the theme renders the post
    # title as the page H1. The gate enforces the declared count.
